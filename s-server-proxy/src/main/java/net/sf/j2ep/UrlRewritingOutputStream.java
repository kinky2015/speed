...
public final class UrlRewritingOutputStream extends ServletOutputStream {
    ...
    private static Pattern linkPattern = Pattern
            .compile(
                    "\\b(url\\s*[=\\:]|ajaxurl\\s*=|href\\s*=|src\\s*=|action\\s*=|url\\()\\s*([\"\'])(([^/]+://)([^/<>]+))?([^\"\'>]*)[\"\']",
    ...
    public UrlRewritingOutputStream(ServletOutputStream originalStream,
            String ownHostName, String contextPath, ServerChain serverChain) {
    ...
    public void rewriteRss(Server server) throws IOException {
        String streamString = stream.toString();
        if (streamString.startsWith("<?xml")) {
            String originalPath = server.getDomainName() + server.getPath();
            ...
            String replacement = this.ownHostName + this.contextPath;
            ...
            originalStream.write(streamString.replaceAll(originalPath,
                    replacement).getBytes());
        } else {
            originalStream.write(streamString.getBytes());
        }
    }
     ...
     * @param link
     *            The part of the link after the domain name
    ...
    private String handleExternalLink(Matcher matcher, String link) {
        ...
        if (matchingServer != null) {
            ...
            return type + separator + protocol + ownHostName + contextPath
                    + link + separator;
        } else {
            return null;
        }
